...
import com.example.ddm.appui.adapter.FlowIndicator;
...
//			}
	...
	public boolean dispatchTouchEvent(MotionEvent ev) {
		//可以获取到down事件
//        Log.i("RollViewpager", "dispatchTouchEvent:"+ev.getAction());
		switch (ev.getAction()) {
			case MotionEvent.ACTION_DOWN:
				downX = (int) ev.getX();
				downY = (int) ev.getY();
				//子viewpager可以左右滑动
				//大喊一声，爹，不要抢事件 (请求爹不允许中断事件)
				getParent().requestDisallowInterceptTouchEvent(true);
				break;
			case MotionEvent.ACTION_MOVE:
				//如果是左右滑动，还继续以下逻辑 ，如果是上下滑动，应该让父亲处理
				int moveX = (int) ev.getX();
				int moveY = (int) ev.getY();
				int disX = Math.abs(moveX - downX);
				int disY = Math.abs(moveY - downY);
				if(disX > disY){
					//如果是最后一个条目，并且是左滑，应该让父亲中断事件
					if(mCurrentAdapterIndex == 3 && downX>moveX){
						getParent().requestDisallowInterceptTouchEvent(false);
					}else if(mCurrentAdapterIndex == 0 && moveX>downX){
						//如果是第一个条目，并且是右滑，应该让父亲中断事件
						getParent().requestDisallowInterceptTouchEvent(false);
					}else{
						//else  自己处理
						getParent().requestDisallowInterceptTouchEvent(true);
